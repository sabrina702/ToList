...
//------------------------------------------------------------  
    ...
    addAtIndex(posicao, data) {// add em qualquer ponto 
      if(posicao<=0)//add no começo 
        return this.addFirst(data);
      if(posicao>=this.length)//add no final 
        return this.addLast(data);
      const newNode = new Node(data);
      let noAtual = this.head;
      let posAtual = 0;
      while(posAtual < posicao-1){//add na posição escolhida 
